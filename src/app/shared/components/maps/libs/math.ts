import { Degrees, MetersCoords } from './libs-interface';
...
//
export const calculateBounds = (minLat: number, maxLat: number, minLon: number, maxLon: number): LngLatBounds => {
  ...
  return [
    ...
    [bottomLon + bottomLon * ADDITIONAL_INDEX, bottomLat + bottomLat * ADDITIONAL_INDEX],
  ];
};
...
//
export const getMaxZoomForBounds = (bounds: LngLatBounds, mapSizePx: number) => {
  ...
  const topMercator = Math.log(Math.tan(Math.PI / 4 + (bottomLat * Math.PI) / 180 / 2));
  const bottomMercator = Math.log(Math.tan(Math.PI / 4 + (topLat * Math.PI) / 180 / 2));
  const deltaMercator = Math.abs(topMercator - bottomMercator);
  const zoomY = Math.log2(((mapSizePx / deltaMercator) * (2 * Math.PI)) / BASE_WORLD_SIZE);
